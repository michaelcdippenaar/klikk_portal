        ...
        <div v-else class="pivot__table-wrap">
          <table class="pivot-table">
            <thead>
              <tr>
                <th
                  v-for="(rh, i) in pivot.rowHeaders"
                  :key="`rh-${i}`"
                ...
                <th
                  v-for="(col, ci) in pivot.colHeaders"
                  :key="`col-${ci}`"
            ...
            <tbody>
              <tr v-for="(row, ri) in pivot.rows" :key="`row-${ri}`">
                <th
...
function readCell(cell) {
  ...
  // object: { value } | { Value } | { v } | { Numeric }
  const v = cell.value ?? cell.Value ?? cell.v ?? cell.Numeric ?? cell.numeric;
  return v ?? null;
}
...
function seedDefaultMember(dim, options) {
  ...
  if (assignments[dim] === 'filter') {
    if (!filterSelections[dim]) filterSelections[dim] = top;
...
// ── Number formatting (ZAR-agnostic — raw values, thousands-separated) ──────
...
function formatCell(value) {
  ...
  return String(value); // non-numeric cell (string member) — show as-is.
}
...
.pivot__dim {
  display: grid;
  grid-template-columns: minmax(140px, 1fr) 160px minmax(220px, 2fr);
  align-items: start;
  gap: 12px;
  padding: 12px;
  border: 1px solid var(--kdl-border-subtle);
